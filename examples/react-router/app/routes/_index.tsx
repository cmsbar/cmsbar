...
// components the Next example uses, driven here by the React Router host seam.
//
// The route `meta` is built from the CMS-editable page metadata: resolvePageMeta
// (neutral) merges the "home" pageMeta entry over a hardcoded fallback, and we
// map that plain object onto React Router meta descriptors.
...
export function meta(_args: Route.MetaArgs) {
  const resolved = resolvePageMeta(getPageMeta("home"), {
    title: "CMSBar runs in this React Router host",
  ...
  if (resolved.ogImage) {
    tags.push({ property: "og:image", content: resolved.ogImage });
  }
  if (resolved.canonical) {
    tags.push({ tagName: "link", rel: "canonical", href: resolved.canonical });
  }
  if (resolved.robots) {
