<script lang="ts">
  ...
  // template/components/cmsbar/EditableImage.tsx to Svelte 5. It lists the
  // image library (GET /images/list from the active branch), merges pending
  ...
  //
  // Rendered through a Svelte portal (the `portal` action moves the root node to
  // <body>) inside a fixed full-screen overlay, mirroring the React Portal so the
  // modal escapes any transformed/overflow-clipped ancestor.
  ...
  // Mounted fresh per open (keyed by {#if} in the parent), so the starting
  // folder only needs currentSrc's open-time value - untrack makes that explicit.
  let selectedFolder = $state(untrack(() => initialFolder(currentSrc)));
  ...
  // useMemo chain: pending uploads appear in their target folder, pending
  // deletes hide their file, pending folder-creates appear in the tree.
  const rootRe = new RegExp(`^${MEDIA_ROOT}/`);
  const allEntries = $derived.by<ImgEntry[]>(() => {
    const out = [...(images ?? [])];
  ...
  function onUpload(file: File) {
    error = null;
    try {
      store.addUpload(contentPath, file, selectedFolder);
      onClose();
    } catch (e) {
      error = humanError(e);
    }
  ...
  function onPickExisting(img: ImgEntry) {
    error = null;
    store.addEdit(contentPath, img.path);
    onClose();
  }
  ...
  function onCreateFolder() {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
      error = "Folder name must contain only letters, numbers, '-' or '_'";
      return;
    }
  ...
  function onDelete(img: ImgEntry) {
    ...
    images = images?.filter((p) => p.repoPath !== img.repoPath) ?? null;
  }
  ...
  // Svelte portal: relocate the node to <body> on mount, restore body scroll
  // lock on destroy. Equivalent to the React <Portal> + body-overflow effect.
...
>
  ...
  >
    ...
    <div class="cmsbar-mb-body">
      <aside class="cmsbar-mb-folders">
        <div class="cmsbar-mb-folders-head">
          <span>Folders</span>
          <button type="button" onclick={onCreateFolder}>+ New</button>
        </div>
        <ul>
          {#each folders as f (f)}
            {@const depth = f.split("/").length - 1}
            {@const label = f === "images" ? "/" : f.split("/").pop()}
            {@const isNew = store.pendingFolders.some(
              (p) => p.replace(/^\/+|\/+$/g, "") === f,
            )}
            <li>
              <button
              ...
              >
                {label}
                {#if isNew}<span class="cmsbar-mb-folder-new">new</span>{/if}
              </button>
            </li>
          {/each}
        </ul>
      ...
      <div class="cmsbar-mb-grid-wrap">
        {#if !images}
          <p class="cmsbar-mb-muted">Loading…</p>
        {:else if filtered.length === 0}
          <p class="cmsbar-mb-muted">
            No images in this folder. Use <strong>Upload to this folder</strong>
            above.
          </p>
        {/if}
        <div class="cmsbar-mb-grid">
          {#each filtered as img (img.repoPath)}
            {@const current = img.path === currentSrc}
            {@const pending = isPendingUpload(img.repoPath)}
            <div class="cmsbar-mb-card" class:current>
              <img
              ...
              />
              {#if pending}
                <span class="cmsbar-mb-pending">Pending</span>
              {/if}
    ...
    <div class="cmsbar-mb-foot">
      Nothing here commits to the repo by itself - uploads, picks, folder creates
      and deletes all queue as pending changes. Click <strong>Save changes</strong
      > in the CMS bar to commit them as one PR.
    </div>
  </div>
</div>
