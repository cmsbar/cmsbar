// Unit tests for the CMSBar editing store (review-mode behaviour).
//
// These exercise the pure store API without mounting a component or calling
// store.start(), so no $effect scheduling is involved - every mutating action
// updates $state and persists synchronously, which keeps the tests
// deterministic. IndexedDB is absent under jsdom and uploadStorage guards on
// that, so its calls are harmless no-ops. We install a controllable
// localStorage mock to assert exactly what is (and isn't) written.
...
beforeEach(() => {
  ...
  // The store also reads window.localStorage in its persistence helpers.
...
describe("addEdit", () => {
  ...
  it("counts folders and deletes too", () => {
    ...
    store.addDelete("public/static/images/old.jpg");
    expect(store.pendingCount).toBe(3);
    expect(store.pendingFolders).toEqual(["images/sub"]);
    expect(store.pendingDeletes).toEqual(["public/static/images/old.jpg"]);
  });
});
...
describe("review-mode persistence", () => {
  it("keeps committed overrides in localStorage (committed view survives reload)", () => {
    ...
    expect(raw, "review mode should still persist committed overrides").toBeTruthy();
