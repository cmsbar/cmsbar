...
//
// In direct publishing the draft branch IS the base branch, so its localStorage
// key is shared by every future session on this device. After a commit the store
// must persist ONLY overrides that are still pending - committed values already
// live on the base branch, and keeping them here would shadow newer live content
// and silently re-publish stale values later.
//
...
// config module synchronously, supplying both things this test's import chain
// needs: publishingMode (forced "direct") and defineCmsConfig (used by
// @/cms.config at its own import time), plus mediaRootDir (media.ts reads the
// static-asset root through it via the store's clampFolder/MEDIA_ROOT import).
...
import { createCmsStore } from "./content";
...
describe("DIRECT-mode persistence rule", () => {
  it("does NOT keep committed overrides in localStorage after applyCommitted", () => {
    ...
    // ...but the persisted entry must NOT carry the committed override (it is no
    // longer pending). With no other pending state the key is removed.
