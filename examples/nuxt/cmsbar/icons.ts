// Inline-SVG icon registry for the Vue CMSBar primitives, ported 1:1 from
// examples/sveltekit/src/cmsbar/icons.svelte.ts (itself mirroring
// template/components/cmsbar/icon-registry.ts).
//
// The React template registry maps names -> lucide-react components. lucide-react
// is React-only, so rather than add a Vue icon dependency this module embeds the
// exact lucide node arrays (path/circle/rect/... primitives, copied verbatim from
// lucide's source) and renders them through Icon.vue. The rendered SVG is
// byte-for-byte the lucide glyph (same viewBox, stroke, paths), so the editor's
// icon vocabulary stays in lockstep with the React app.
//
// Parity contract with icon-registry.ts:
//   - ICON_NAMES: same ordered set of names (Clock first = the add-item default,
//     and getIconNode's fallback).
