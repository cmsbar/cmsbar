...
// template/components/cmsbar/SettingsDrawer.tsx (mirroring SettingsDrawer.svelte)
// to a Vue 3 SFC. Keeps the framework-neutral core: the publishing-mode display,
// the launch-gate section (via the neutral siteLaunchLabel/SiteLaunch helpers),
// and the shared-highlight toggle the bar owns. Edits stage into the active draft
// via store.addEdit. Rendered via <Teleport to="body"> + body-scroll lock.
//
// The React `onToggleHighlight?: () => void` (undefined => hide the section)
// becomes a `canToggleHighlight` boolean prop + a `toggle-highlight` emit.
...
const props = withDefaults(
  ...
  { highlightShared: false, canToggleHighlight: false },
);
...
const status = computed(() => siteLaunchLabel(launch.value));

function update(patch: Partial<SiteLaunch>) {
  store.addEdit("launch", { ...launch.value, ...patch });
}
...
<template>
  <Teleport to="body">
    <div class="cmsbar-drawer-root" data-cms-ui>
      <div class="cmsbar-drawer-scrim" role="presentation" @click="emit('close')"></div>
      <div class="cmsbar-drawer" role="dialog" aria-modal="true" aria-label="Settings">
        ...
        <div class="cmsbar-drawer-body">
          <div v-if="!canEdit" class="cmsbar-ro-note">
          ...
          <section class="cmsbar-card">
            ...
            <div class="cmsbar-card-pad">
              <div class="cmsbar-status-row" :class="{ live: DIRECT, teaser: !DIRECT }">
                {{
                  DIRECT
          ...
          <section class="cmsbar-card">
            ...
            <fieldset class="cmsbar-card-pad cmsbar-fieldset" :disabled="!canEdit">
              <div
                ...
                :class="{ live: status.tone === 'live', teaser: status.tone === 'teaser' }"
              >
                {{ status.text }}
              </div>
              ...
              <div class="cmsbar-radio-stack">
                <button
                  v-for="m in LAUNCH_MODES"
                  :key="m.value"
                  type="button"
                  class="cmsbar-radio"
                  :class="{ active: launch.mode === m.value }"
                  :aria-pressed="launch.mode === m.value"
                  @click="update({ mode: m.value })"
                >
                  <span class="cmsbar-radio-dot" :class="{ active: launch.mode === m.value }">
                    <span v-if="launch.mode === m.value" class="cmsbar-radio-fill"></span>
                  </span>
                  <span>
          ...
          <section v-if="canToggleHighlight" class="cmsbar-card">
            <div class="cmsbar-card-head">
        ...
        <footer class="cmsbar-drawer-foot">
          <template v-if="canEdit">
            Changes stage into your draft. Click <strong>Save</strong> in the CMS
            bar to commit them.
          </template>
          <template v-else> Read-only. Start a draft to edit. </template>
        </footer>
      </div>
    </div>
  </Teleport>
