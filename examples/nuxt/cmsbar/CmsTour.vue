...
// (mirroring CmsTour.svelte) to a Vue 3 SFC. Renders nothing unless the project
...
// TOUR_OPEN_EVENT - the bar's "✦ Guide" pill does exactly that. A window event
// (not lifted state) keeps the bar's render branches untouched.
//
// TOUR_OPEN_EVENT is exported from this plain <script> block (the Vue analogue of
// Svelte's `<script module>`), so CmsBar can import it from this .vue module.
...
const step = computed(() => (open.value ? steps[index.value] : undefined));
...
// Imperative open (bar button) + first-visit auto-open + resume an in-flight
// tour across the reloads its own steps cause. Resume must NOT be gated on
// autoStart/done. Mirrors CmsTour.svelte's mount effect.
function onOpenEvent() {
  index.value = 0;
  open.value = true;
}
onMounted(() => {
  window.addEventListener(TOUR_OPEN_EVENT, onOpenEvent);
  if (steps.length > 0) {
    try {
      const saved = sessionStorage.getItem(STEP_SS_KEY);
      if (saved !== null) {
        const n = Number(saved);
        index.value = Number.isInteger(n)
      ...
      if (cmsConfig.tour?.autoStart) open.value = true;
    }
  }
});
onBeforeUnmount(() => {
  window.removeEventListener(TOUR_OPEN_EVENT, onOpenEvent);
});

// Track the target's rect; recompute on scroll/resize. Capture-phase scroll
// listener so nested scroll containers are covered too. Re-arms on step change.
watch(
  step,
  (s, _old, onCleanup) => {
    if (!s) return;
    try {
      sessionStorage.setItem(STEP_SS_KEY, String(index.value));
    } catch {
      /* storage unavailable */
    }
...
watch(open, (isOpen, _old, onCleanup) => {
  if (!isOpen) return;
  const onKey = (e: KeyboardEvent) => {
    if (e.key !== "Escape") return;
    const t = e.target as HTMLElement | null;
    if (t?.closest?.("input, textarea, [contenteditable], [data-cms-toolbar]")) {
      return;
    }
    close(true);
  };
...
<template>
  <Teleport to="body">
    ...
    <div v-if="step" class="cmsbar-tour-layer" data-cms-ui>
      <div
        v-if="rect"
      ...
      ></div>
      <div v-else class="cmsbar-tour-scrim"></div>
      <div
        ref="cardEl"
      ...
      >
        ...
        <div class="cmsbar-tour-actions">
          <button type="button" class="cmsbar-tour-skip" @click="close(true)">
            Skip tour
          </button>
          <button
            v-if="index > 0"
