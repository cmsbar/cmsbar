...
// The TanStack Start equivalents live in src/cmsbar/ (committed).
//
// We also do NOT copy the template's app/ tree (app/api/cms, app/cmsbar): those
// are Next route handlers / pages. TanStack Start mounts the whole API through
// one catch-all server route (src/routes/api/cms.$.ts -> handleCmsRequest).
//
...
//
// Mirrors examples/react-router/scripts/setup.mjs; the only differences are the
// destination root (src/ instead of app/) and the resolution of "@/..." onto it.
