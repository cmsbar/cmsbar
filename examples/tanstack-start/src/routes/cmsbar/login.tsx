// CMSBar login page (TanStack Start). Plain client-side fetch to the CMS login
// endpoint, then a full-page navigation to "/" so the root loader re-reads the
// freshly set session cookie on the server and renders the authenticated bar.
// Mirrors the template's app/cmsbar/login/page.tsx and the React Router
// example's cmsbar.login.tsx.

import { useState } from "react";
import { createFileRoute } from "@tanstack/react-router";
import { cmsFetch } from "@/lib/cmsbar/cmsFetch";

export const Route = createFileRoute("/cmsbar/login")({
  head: () => ({ meta: [{ title: "CMSBar login" }] }),
...
function CmsLoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Full-page navigation so the server-rendered root re-reads the new cookie.
