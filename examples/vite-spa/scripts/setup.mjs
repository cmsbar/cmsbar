...
// A client-only SPA needs NO framework host adapter at all: the neutral
// host.tsx (HostProvider with DOM defaults - window.location + popstate + a
// plain <img>) is enough. So unlike the React Router / TanStack examples there
// is no src/cmsbar/ host file - main.tsx wires <HostProvider> directly.
//
// We also do NOT copy the template's app/ tree (app/api/cms, app/cmsbar): those
// are Next route handlers / pages. The SPA's companion server mounts the whole
// API through one Hono catch-all (server/index.ts -> createCmsApi()).
//
