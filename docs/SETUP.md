...
- **Login rate-limiting trusts client-IP headers.** The per-IP login lockout (5 failures / 15 min) keys on the client IP that `clientIp()` (`lib/cmsbar/rateLimit.ts`) reads from request headers - it prefers `x-real-ip`, then `cf-connecting-ip`, and only falls back to the leftmost `X-Forwarded-For` when neither is present. The leftmost `X-Forwarded-For` is client-supplied and spoofable, so if you deploy behind a **custom reverse proxy** you must ensure it sets a trusted single-value client-IP header (`x-real-ip`) and **strips any inbound copy** of these headers from the request; otherwise an attacker can rotate the header per request and bypass the lockout. Coolify/Traefik and Cloudflare already set these for you.
